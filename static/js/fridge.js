...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Fridge location buttons
  document.querySelectorAll('.fridge-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
      const target = btn.getAttribute('data-target');
      switchFridgeLocation(target);
    });
  });
...
// Switch between fridge locations
function switchFridgeLocation(location) {
...
function renderItems() {
  // Get the container
  const itemsContainer = document.getElementById('fridge-items-container');
  
  // Check if the container exists
  if (!itemsContainer) {
    console.error('Error: fridge-items-container element not found');
    return;
  }
  
  itemsContainer.innerHTML = '';
  
  // Filter items for the selected fridge location
  let fridgeItems = [];
  
  if (state.activeLocation === 'all') {
    // Show all fridge items
    fridgeItems = state.items.filter(item => item.location === 'fridge');
  } else {
    // Show items for specific fridge location
    fridgeItems = state.items.filter(item => {
      if (item.location !== 'fridge') return false;
      
      // Handle both formats: "kitchen" and "kitchen fridge"
      const fridgeLoc = item.fridgeLocation ? item.fridgeLocation.toLowerCase() : '';
      return fridgeLoc === state.activeLocation || 
             fridgeLoc === `${state.activeLocation} fridge` ||
             fridgeLoc.startsWith(state.activeLocation);
    });
  }
  ...
  if (state.activeSubcategory !== 'all') {
    fridgeItems = fridgeItems.filter(item => {
      ...
      const subcategoryMap = {
        ...
        'meat': ['meat', 'chicken', 'beef', 'pork', 'poultry'],
        'vegetables': ['vegetables', 'vegetable', 'veg', 'veggies'],
        'fruit': ['fruit', 'fruits', 'berries', 'berry'],
        'drinks': ['drinks', 'drink', 'beverage', 'beverages', 'juice', 'soda'],
        'condiments': ['condiments', 'condiment', 'sauce', 'dressing'],
        'leftovers': ['leftovers', 'leftover', 'meal', 'prepared'],
        'dessert': ['dessert', 'desserts', 'sweet', 'sweets'],
        'snacks': ['snacks', 'snack'],
        'prepared': ['prepared', 'ready', 'meal', 'meals']
      };
  ...
  if (fridgeItems.length === 0) {
    ...
    if (state.activeLocation === 'all' && state.activeSubcategory === 'all') {
      message = 'No items found in any fridge. Add some items!';
    } else if (state.activeLocation === 'all') {
      message = `No ${state.activeSubcategory} items found in any fridge.`;
    } else if (state.activeSubcategory === 'all') {
      const locationName = `the ${state.activeLocation} fridge`;
    ...
    <div class="item-content">
      ...
      <div class="item-details">
        <div class="item-detail">
          ...
          <span>${item.fridgeLocation || 'Fridge'}</span>
        </div>
        ${subcategoryHtml}
        <div class="item-detail">
...
async function handleAddItem(e) {
  ...
  const fridgeLocation = document.getElementById('fridgeLocation').value;
  ...
  const newItem = {
    name,
    subcategory,
    fridgeLocation: `${capitalizeFirstLetter(fridgeLocation)} Fridge`,
    packType: packType,
    quantity,
...
async function handleImportItems(e) {
  ...
  const fridgeLocation = document.getElementById('importFridgeLocation').value;
  ...
  try {
    ...
    formData.append('location', 'fridge');
    formData.append('freezerLocation', fridgeLocation);
...
function updateRemoveItemDropdown() {
  ...
  let filteredItems = state.items.filter(item => item.location === 'fridge');
  
  // Filter by location if not "all"
  if (selectedLocation !== 'all') {
    filteredItems = filteredItems.filter(item => {
      const fridgeLoc = item.fridgeLocation ? item.fridgeLocation.toLowerCase() : '';
      return fridgeLoc === selectedLocation || 
             fridgeLoc === `${selectedLocation} fridge` ||
             fridgeLoc.startsWith(selectedLocation);
    });
  }
